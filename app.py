...
# IMPORTE NO TERMINAL DO VSCODE TODAS AS BIBLIOTECAS NECESSÁRIAS ABAIXO:
...
# (Dentro da função processar_documento_combinado)
    ...
    --- REGRAS FUNDAMENTAIS ---
    1.  Sua resposta deve se basear **ESTRITA E EXCLUSIVAMENTE** no conteúdo dos textos fornecidos. É **PROIBIDO** usar conhecimento externo.
    2.  Analise **CADA DOCUMENTO DE FORMA INDEPENDENTE**. Não misture os resumos ou dados de um documento com outro.
    3.  **GERAÇÃO AUTOMÁTICA DE GRÁFICOS:** Se um documento contiver dados quantitativos claros (comparações, percentuais, valores ao longo do tempo), você **DEVE** gerar um objeto de gráfico para ele no `BLOC-DASHBOARD-JSON`.
    4.  Se não for possível extrair informações para uma seção, use a frase **"Informação Não Processada"**.
    5.  A geração de uma tabela de dados HTML em cada resumo de documento é **OBRIGATÓRIA**.

    {instrucao_foco}

    Sua tarefa é analisar os textos dos documentos fornecidos e gerar uma resposta consolidada em dois blocos: um HTML com resumos separados para cada documento, e um JSON com dados de dashboard separados para cada documento.
    ...
    <details class="resumo-documento" open>
        ...
        <div class="conteudo-documento">
            <p><strong>Resumo Executivo deste Documento:</strong> [Resumo focado APENAS neste documento].</p>
            <h4>Tabela de Dados Extraídos:</h4>
            [Crie OBRIGATORIAMENTE uma tabela HTML com os dados deste documento].
        </div>
    </details>
    ...
    ### BLOC-DASHBOARD-JSON ###
    {{
        "[nome_do_primeiro_arquivo.pdf]": [
            {{
                "tipo_visualizacao_sugerida": "grafico_barras",
                "titulo": "[Título do gráfico para este arquivo]",
                "dados": {{ "labels": ["[Categoria 1]"], "valores": ["[Valor 1]"] }}
            }},
            {{
                "tipo_visualizacao_sugerida": "kpi",
                "titulo": "[Título do KPI para este arquivo]",
                "valor": "[Valor]",
                "descricao": "[Descrição]"
            }}
        ],
        "[nome_do_segundo_arquivo.docx]": [
            {{
                "tipo_visualizacao_sugerida": "tabela",
                "titulo": "[Título da tabela para este arquivo]",
                "dados": {{
                  "colunas": ["Coluna A", "Coluna B"],
                  "linhas": [["dado1A", "dado1B"]]
                }}
            }}
        ]
    }}
